...
#### 2. **多台服务器轮换切换机制**
- ✅ **主备架构**：1 台主服务器 + N 台备用服务器，主节点异常自动切换。
- ✅ **健康检查**：每 5 秒检测业务节点状态（HTTP 探活 + WebSocket 连接测试）。
- ✅ **自动故障转移**：主节点无响应 >15 秒，自动切换至备用节点，全程 <3 秒。
- ✅ **Nginx upstream**：多节点负载均衡，故障节点自动剔除，恢复后自动加入。
- ✅ **无缝迁移**：WebSocket 连接断开后自动重连至新节点，用户无感知。
- ✅ **数据同步**：Redis 共享会话状态，切换后用户交易数据不丢失。
